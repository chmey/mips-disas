...
# Dictionary of registers holding their values
# Index by numerical representation
regval = {}
...
def emulate(instruction_bytes):
    global reg_ip
    """
    Emulate a MIPS instruction and update the state.
    """
    op_code = get_opcode(instruction_bytes)
    if op_code == 0:
        # Use ALU
        R_alu(instruction_bytes)
    elif op_code > 0x03:
        I_alu(instruction_bytes)
    else:
        print(f"Emulator not implemented for opcode {op_code}")
        exit(1)
...
def I_alu(instruction_bytes):
    global reg_ip
    """
    Emulate some I type instructions.
    ...
    if op_code == 0x9:
        # addiu
        regval[rt] = (regval[rs] + sign_extend(imm)) & 0xFFFFFFFF
    elif op_code == 0xD:
        # ori
        regval[rt] = regval[rs] | imm
    elif op_code == 0xF:
        # lui
        regval[rt] = imm << 16
    else:
        print(f"I {op_code} not implemented")
        exit(1)


def R_alu(instruction_bytes):
    global reg_ip
    """
    Emulate a MIPS ALU for some R type instructions.
    Modify register values.
    """
    fun = get_function_code(instruction_bytes)
    rd = (instruction_bytes & 0x000F800) >> 11
    rt = (instruction_bytes & 0x01F0000) >> 16
    if fun == 0x00:
        # sll
        shamt = (instruction_bytes & 0x000007C0) >> 6
        regval[rd] = (regval[rt] << shamt) & 0xFFFFFFFF
    elif fun == 0x25:
        # or
        rs = (instruction_bytes & 0x3E00000) >> 21
        ...
        # subu
        rs = (instruction_bytes & 0x3E00000) >> 21
        regval[rd] = (regval[rs] - regval[rt]) & 0xFFFFFFFF
    elif fun == 0x27:
        # nor
        rs = (instruction_bytes & 0x3E00000) >> 21
        regval[rd] = ~(regval[rs] | regval[rt])
...
def exec_loop(data):
    ...
    while(reg_ip - entry_addr < len(data)):
        ...
        emulate(instruction_bytes)
        # increment IP
        reg_ip += 4


def set_state(a, b, c, d, e, f):
    """
    Set registers as specified by the requirement.
    """
    regval[20] = a  # s4
    regval[19] = b  # s3
    regval[21] = c  # s5
...
def get_state():
    """
    Get variables from registers as required.
    """
    print(f"a: 0x{regval[20]:08x}")
    print(f"b: 0x{regval[19]:08x}")
    print(f"c: 0x{regval[21]:08x}")
    print(f"d: 0x{regval[18]:08x}")
    print(f"e: 0x{regval[17]:08x}")
    print(f"f: 0x{regval[16]:08x}")


def make_flag():
    """
    Make flag from variables.
    """
    return f"{regval[20]:08x}{regval[19]:08x}{regval[21]:08x}{regval[18]:08x}{regval[17]:08x}{regval[16]:08x}"


# Setup example state
set_state(0x100, 0x200, 0x300, 0x400, 0x500, 0x600)
...
# Verify with provided exemplary solution
assert (make_flag() == "00000102000001ff0000060000002e0555786ffbec0c1016")
